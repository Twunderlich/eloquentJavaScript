...
// chapter 5 - Mother-child age difference  - complete!

// required dataset not working. completed on website.

//
// require("./lib/ancestry.js")
...
function isInArray( moth ) {
  return byName[ moth ] !== undefined;
};

function avgAgeOfBirths( obj ) {
  	var agesArray = [];
	for ( var person in obj ) {
      var per = obj[ person ];
      if ( isInArray( per.mother ) ) {
        agesArray.push( per.born - byName[ per.mother ].born )
      }
    }
  return average( agesArray ).toFixed( 1 );
}

// console.log( avgAgeOfBirths( byName ) );

// → 31.2

// chapter 5 - Historical life expectancy - complete!
//
...
// taking their year of death, dividing it by 100, and rounding it up, as in Math.ceil(person.died / 100).g
...
function avgAgePerCentury( arr ) {
	var centuries = arr.reduce( function ( obj, person ) {
      var century = Math.ceil( person.died / 100 );
      if ( obj[ century ] === undefined ) {
        obj[ century ] = ( person.died - person.born );
      } else {
      	obj[ century ] = ( obj[ century ] + ( person.died - person.born ) ) / 2;
      }
      return obj;
    }, {} );
  
	for ( var year in centuries ) {
      	var avgAge = centuries[ year ];
      	if ( !Number.isInteger( avgAge ) ) {
 			avgAge = avgAge.toFixed( 1 );
        }
    	console.log( year + ': ' + avgAge );
    }
}
